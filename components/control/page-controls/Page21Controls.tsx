...
export default function Page21Controls() {
  ...
  const handleVolumeChange = (newVolume: number) => {
    setVolume(newVolume);
    setPageControl(21, { type: 'volume', value: newVolume });
  };

  // 发送视频跳转控制
  const handleSeek = (seconds: number) => {
    setPageControl(21, { type: 'seek', value: seconds });
  };
  ...
  const handleToggleRecording = () => {
    if (isRecording) {
      ...
      setPageControl(21, { type: 'stop-recording' });
    } else {
      // 开始记录
      setIsRecording(true);
      setPageControl(21, { type: 'start-recording' });
    }
  };
