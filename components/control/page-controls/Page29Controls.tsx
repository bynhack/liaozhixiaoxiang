...
export default function Page29Controls() {
  const { isPlaying } = usePresentationStore();
  const { togglePlay, setPageControl } = useControlSocket();
  const [volume, setVolume] = useState(50); // 0-100，第二十九页默认50%音量，有声音

  // 发送音量控制
  const handleVolumeChange = (newVolume: number) => {
    setVolume(newVolume);
    setPageControl(29, { type: 'volume', value: newVolume });
  };

  // 发送视频跳转控制
  const handleSeek = (seconds: number) => {
    setPageControl(29, { type: 'seek', value: seconds });
  };
