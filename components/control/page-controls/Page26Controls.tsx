...
export default function Page26Controls() {
  ...
  const handleVolumeChange = (newVolume: number) => {
    setVolume(newVolume);
    setPageControl(26, { type: 'volume', value: newVolume });
  };

  // 发送视频跳转控制
  const handleSeek = (seconds: number) => {
    setPageControl(26, { type: 'seek', value: seconds });
  };
  ...
  const handleToggleRecording = () => {
    if (isRecording) {
      ...
      setPageControl(26, { type: 'stop-recording' });
    } else {
      // 开始记录
      setIsRecording(true);
      setPageControl(26, { type: 'start-recording' });
    }
  };
