...
export default function Page20Controls() {
  ...
  const handleVolumeChange = (newVolume: number) => {
    setVolume(newVolume);
    setPageControl(20, { type: 'volume', value: newVolume });
  };

  // 发送视频跳转控制
  const handleSeek = (seconds: number) => {
    setPageControl(20, { type: 'seek', value: seconds });
  };
  ...
  const handleToggleRecording = () => {
    if (isRecording) {
      ...
      setPageControl(20, { type: 'stop-recording' });
    } else {
      // 开始记录
      setIsRecording(true);
      setPageControl(20, { type: 'start-recording' });
    }
  };
