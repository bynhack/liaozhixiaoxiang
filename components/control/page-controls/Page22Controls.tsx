...
export default function Page22Controls() {
  ...
  const handleVolumeChange = (newVolume: number) => {
    setVolume(newVolume);
    setPageControl(22, { type: 'volume', value: newVolume });
  };

  // 发送视频跳转控制
  const handleSeek = (seconds: number) => {
    setPageControl(22, { type: 'seek', value: seconds });
  };
  ...
  const handleToggleRecording = () => {
    if (isRecording) {
      ...
      setPageControl(22, { type: 'stop-recording' });
    } else {
      // 开始记录
      setIsRecording(true);
      setPageControl(22, { type: 'start-recording' });
    }
  };
