...
export default function Page31Controls() {
  const { isPlaying } = usePresentationStore();
  const { togglePlay, setPageControl } = useControlSocket();
  const [volume, setVolume] = useState(50); // 0-100，第三十一页默认50%音量，有声音

  // 发送音量控制
  const handleVolumeChange = (newVolume: number) => {
    setVolume(newVolume);
    setPageControl(31, { type: 'volume', value: newVolume });
  };

  // 发送视频跳转控制
  const handleSeek = (seconds: number) => {
    setPageControl(31, { type: 'seek', value: seconds });
  };
