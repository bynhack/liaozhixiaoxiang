...
export default function Page23Controls() {
  ...
  const handleVolumeChange = (newVolume: number) => {
    setVolume(newVolume);
    setPageControl(23, { type: 'volume', value: newVolume });
  };

  // 发送视频跳转控制
  const handleSeek = (seconds: number) => {
    setPageControl(23, { type: 'seek', value: seconds });
  };
  ...
  const handleToggleRecording = () => {
    if (isRecording) {
      ...
      setPageControl(23, { type: 'stop-recording' });
    } else {
      // 开始记录
      setIsRecording(true);
      setPageControl(23, { type: 'start-recording' });
    }
  };
