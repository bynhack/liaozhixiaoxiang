...
export default function Page20() {
  ...
  const lyrics = [
    { text: '两只小象呦啰啰', startTime: 0, endTime: 999 }, // 只有一句歌词，设置一个很长的时间范围
  ];
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (audioInternalPlayer && audioInternalPlayer.currentTime !== undefined) {
        const time = audioInternalPlayer.currentTime;
        setCurrentTime(time);
        ...
        // 如果时间超过所有歌词，显示最后一句
        if (newIndex === -1 && time >= lyrics[lyrics.length - 1].startTime) {
  ...
  // 监听页面控制命令
  usePageControl(20, handlePageControl);
