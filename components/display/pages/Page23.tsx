...
export default function Page23() {
  ...
  const lyrics = [
    { text: '勾一勾呀呦啰啰', startTime: 0, endTime: 999 }, // 只有一句歌词，设置一个很长的时间范围
  ];
  ...
  const extractReferenceWaveform = useCallback(async () => {
    ...
    try {
      const response = await fetch(encodeURI('/assets/videos/勾一勾呀呦啰啰.mp4'));
  ...
  // 第二十三页加载时，确保为暂停状态（不自动播放）并提取基准波形 - 只运行一次
  useEffect(() => {
  ...
  // 监听页面控制命令
  usePageControl(23, handlePageControl);
    ...
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      >
        {showVideo && isShowingRecordedRef.current ? (
          ...
          <ReactPlayer
            ref={videoPlayerRef}
            url={encodeURI("/assets/videos/勾一勾呀呦啰啰.mp4")}
            playing={true}
            loop={false}
            muted={false}
            controls={false}
      ...
      <div style={{ position: 'absolute', opacity: 0, pointerEvents: 'none', zIndex: 0 }}>
        <ReactPlayer
          ref={audioPlayerRef}
          url={encodeURI("/assets/audios/4.勾一勾伴奏.MP3")}
          playing={isPlaying}
          loop={false}
          muted={audioVolume === 0}
          controls={false}
