...
export default function Page26() {
  ...
  const extractReferenceWaveform = useCallback(async () => {
    ...
    try {
      const response = await fetch('/assets/videos/见面握握手第一句.mp4');
  ...
  // 第二十六页加载时，确保为暂停状态（不自动播放）并提取基准波形 - 只运行一次
  useEffect(() => {
  ...
  // 监听页面控制命令
  usePageControl(26, handlePageControl);
