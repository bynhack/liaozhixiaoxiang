...
export default function Page27() {
  ...
  const lyrics = [
    { text: '见面握握手呦啰啰', startTime: 0, endTime: 999 }, // 只有一句歌词，设置一个很长的时间范围
  ];
  ...
  const extractReferenceWaveform = useCallback(async () => {
    ...
    try {
      const response = await fetch('/assets/videos/见面握握手第二句.mp4');
  ...
  // 第二十七页加载时，确保为暂停状态（不自动播放）并提取基准波形 - 只运行一次
  useEffect(() => {
  ...
  // 监听页面控制命令
  usePageControl(27, handlePageControl);
    ...
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      >
        {showVideo && isShowingRecordedRef.current ? (
          ...
          <ReactPlayer
            ref={videoPlayerRef}
            url={encodeURI("/assets/videos/见面握握手第二句.mp4")}
            playing={true}
            loop={false}
            muted={false}
            controls={false}
      ...
      >
        <div
          style={{
            ...
            minWidth: 'min-content', // 最小宽度为内容宽度
          }}
        >
      ...
      <div style={{ position: 'absolute', opacity: 0, pointerEvents: 'none', zIndex: 0 }}>
        <ReactPlayer
          ref={audioPlayerRef}
          url={encodeURI("/assets/audios/8.见面握握手2伴奏.MP3")}
          playing={isPlaying}
          loop={false}
          muted={audioVolume === 0}
          controls={false}
